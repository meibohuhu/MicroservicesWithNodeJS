import {   requireAuth,
  ...
  NotFoundError,
  OrderStatus,
  BadRequestError, } from '@wendy96tickets/common';
...
const EXPIRATION_WINDOW_SECONDS = 1 * 60;    // set up 1 min
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    new OrderCreatedPublisher(natsWrapper.client).publish({
      id: order.id,
      version: order.version,   // include version
      status: order.status,
      userId: order.userId,
      expiresAt: order.expiresAt.toISOString(),    // using string in json
      ticket: {
        id: ticket.id,
        price: ticket.price,
      },
